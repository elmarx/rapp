...
/// Rust application logger. Sets up slog for structured logging
///
/// - enables JSON logging if RUST_LOG_JSON=1 (i.e. put `ENV RUST_LOG_JSON=1` into your Dockerfile)
/// - inits and stdlogger, so crates using `info!()` from the (default) [log-crate](https://crates.io/crates/log) can log messages
...
/// Usage:
/// ```
/// use rapp_core::{RappLogger, set_global_logger};
///
/// let root_logger = RappLogger::new("service-name").init();
...
impl RappLogger {
    ...
    /// enable debug-log for the given module.
